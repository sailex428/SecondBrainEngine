    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        for (BlockPos pos : locations) {
            ...
            if (state.getBlock() instanceof Fertilizable) {
                ...
                if (ig.isFertilizable(ctx.world(), pos, state  /*? >=1.21.1  {*/ /**/ /*?} else {*/, false/*?}*/) && ig.canGrow(ctx.world(), ctx.world().random, pos, state)) {
        ...
        for (BlockPos pos : both) {
            ...
            Optional<Rotation> rot = RotationUtils.reachableOffset(ctx.entity(), pos, new Vec3d(pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5), ctx.interactionController().getBlockReachDistance(), false);
            if (rot.isPresent() && isSafeToCancel && baritone.getInventoryBehavior().throwaway(true, soulsand ? this::isNetherWart : this::isPlantable)) {
                HitResult result = RayTraceUtils.rayTraceTowards(ctx.entity(), rot.get(), ctx.interactionController().getBlockReachDistance());
